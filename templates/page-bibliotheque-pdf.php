<?php
/**
 * Template Name: Bibliothèque PDF
...
$intro_text    = crades_get_intro_text( $page_id );
	...
	<section class="bg-brand-navy py-16 lg:py-20">
		<div class="mx-auto max-w-6xl px-4 sm:px-6">
			<nav class="mb-4 text-xs text-gray-400">
				...
				<span class="text-gray-300"><?php echo esc_html( get_the_title( $page_id ) ); ?></span>
			</nav>
			...
			<div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
				<div class="max-w-2xl">
					...
					<h1 class="mt-4 font-display text-2xl text-white lg:text-3xl"><?php echo esc_html( get_the_title( $page_id ) ); ?></h1>
					<p class="mt-2 text-sm leading-relaxed text-gray-400">
						<?php echo esc_html( $intro_text ? $intro_text : __( 'Consultez, filtrez et lisez les rapports PDF du CRADES directement sur le site.', 'crades-theme' ) ); ?>
					</p>
				</div>
				...
				<div class="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-white/80 backdrop-blur-sm">
					...
					<span class="text-sm font-semibold text-white" data-pdf-count aria-live="polite"><?php echo esc_html( (string) $library_query->found_posts ); ?></span>
				</div>
			</div>
		</div>
	</section>
		...
		<div class="mx-auto max-w-6xl px-4 sm:px-6">
			...
			<?php else : ?>
				<div class="rounded-xl border border-dashed border-gray-200 bg-white px-6 py-12 text-center text-gray-500">
					...
					<p class="mt-3 text-sm">
						<?php esc_html_e( 'Ajoutez des contenus dans le type de contenu Rapports pour alimenter cette bibliothèque.', 'crades-theme' ); ?>
					</p>
				</div>
			<?php endif; ?>
		</div>
	...
	<div class="fixed inset-0 z-[80] hidden items-center justify-center bg-transparent p-4" data-pdf-modal aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="crades-pdf-reader-title">
		...
		<div class="relative flex h-[82vh] w-[min(95vw,1100px)] flex-col overflow-hidden border border-gray-200 bg-white shadow-2xl">
			<div class="flex items-center justify-between gap-4 border-b border-gray-200 px-4 py-4 sm:px-6">
				<div class="min-w-0">
					<p class="text-xs font-semibold uppercase tracking-[0.24em] text-brand-gold">
						<?php esc_html_e( 'Lecteur PDF', 'crades-theme' ); ?>
					</p>
					<h2 id="crades-pdf-reader-title" class="truncate text-base font-semibold text-slate-900 sm:text-lg" data-pdf-reader-title>
						<?php esc_html_e( 'Document', 'crades-theme' ); ?>
					</h2>
				</div>
				<div class="flex items-center gap-2">
					<a class="inline-flex items-center rounded-full border border-gray-200 px-4 py-2 text-sm font-semibold text-slate-600 transition-colors hover:border-brand-blue hover:text-brand-blue" href="#" data-pdf-reader-download target="_blank" rel="noopener">
						<?php esc_html_e( 'Télécharger', 'crades-theme' ); ?>
					</a>
					<button type="button" class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 text-slate-500 transition-colors hover:border-brand-blue hover:text-brand-blue" data-pdf-close aria-label="<?php esc_attr_e( 'Fermer le lecteur', 'crades-theme' ); ?>">
			...
			<div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3 sm:px-6">
				<div class="flex items-center gap-2">
					<button type="button" class="inline-flex items-center rounded-full border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-600 transition-colors hover:border-brand-blue hover:text-brand-blue disabled:cursor-not-allowed disabled:opacity-40" data-pdf-prev>
						<?php esc_html_e( 'Précédent', 'crades-theme' ); ?>
					</button>
					<button type="button" class="inline-flex items-center rounded-full border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-600 transition-colors hover:border-brand-blue hover:text-brand-blue disabled:cursor-not-allowed disabled:opacity-40" data-pdf-next>
						<?php esc_html_e( 'Suivant', 'crades-theme' ); ?>
					</button>
				</div>
				<p class="text-sm font-medium text-slate-500" data-pdf-page-indicator>
					<?php esc_html_e( 'Page 1 sur 1', 'crades-theme' ); ?>
				</p>
			</div>
			...
			<div class="relative flex-1 overflow-auto bg-slate-100 p-3 sm:p-6" data-pdf-reader-stage>
				...
				<div class="hidden absolute inset-0 items-center justify-center bg-slate-100 px-6 text-center" data-pdf-reader-error>
					<div>
						...
						<p class="mt-2 text-sm text-slate-500" data-pdf-reader-error-message>
							<?php esc_html_e( 'Impossible de charger ce PDF.', 'crades-theme' ); ?>
						</p>
					</div>
				</div>
